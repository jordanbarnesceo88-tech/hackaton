...
test("справочник показывает источники, выведенные из данных", async ({ page }) => {
  ...
  // Двадцать цитат: у трёх вендорских решений цена и производительность — РАЗНЫЕ
  // утверждения (страница продукта цитирует производительность, цена оценочная и ссылки не
  // имеет), плюс по два источника у каждого из семи классов. Число выведено из тех же
  // модулей, что и сев, поэтому разойтись с данными не может.
