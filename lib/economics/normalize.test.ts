...
describe("computeQuantity", () => {
  ...
  categorySlug: "test-task", workerOutputPerYear: 12500, workloadStream: "OPERATION_FLOW", ...base, capacityPerUnit: 50, capacityBasis: "PER_HOUR_FLOW" };
  ...
  it("sizes a PER_DAY_FLOW solution by annualized throughput", () => {
    ...
    const cap: SolutionCapacity = {
  categorySlug: "test-task", workerOutputPerYear: 12500, workloadStream: "OPERATION_FLOW", ...base, capacityPerUnit: 400, capacityBasis: "PER_DAY_FLOW" };
  ...
  it("sizes a CONCURRENT_STOCK solution from an explicit peak", () => {
    ...
    const cap: SolutionCapacity = {
  categorySlug: "test-task", workerOutputPerYear: 12500, workloadStream: "OPERATION_FLOW", ...base, capacityPerUnit: 12, capacityBasis: "CONCURRENT_STOCK" };
  ...
  it("never returns less than 1", () => {
    const cap: SolutionCapacity = {
  categorySlug: "test-task", workerOutputPerYear: 12500, workloadStream: "OPERATION_FLOW", ...base, capacityPerUnit: 999999, capacityBasis: "PER_DAY_FLOW" };
  ...
  it("returns null on non-positive capacity (no divide-by-zero, no throw)", () => {
    const cap: SolutionCapacity = {
  categorySlug: "test-task", workerOutputPerYear: 12500, workloadStream: "OPERATION_FLOW", ...base, capacityPerUnit: 0, capacityBasis: "PER_DAY_FLOW" };
...
describe("capacityPerYear / demandPerYear helpers", () => {
  it("annualizes a PER_HOUR_FLOW capacity using operating hours", () => {
    ...
    const cap = { categorySlug: "test-task", workerOutputPerYear: 12500,
  ...
  it("считает спрос потока площади из areaM2, а не из opsPerDay", () => {
    ...
    // в этот расчёт, число будет не тем, и именно так уборщик «замещал» сорок кладовщиков.
    const params = { areaM2: 10000, opsPerDay: 1600, staffCount: 0 };
    expect(demandPerYear(params, a, "FLOOR_AREA")).toBe(2500000);
    expect(demandPerYear(params, a, "OPERATION_FLOW")).toBe(400000);
  });
});
...
describe("toSolutionCapacity", () => {
  ...
  it("drops the non-engine fields rather than passing them through", () => {
    ...
    expect(Object.keys(toSolutionCapacity(row)).sort()).toEqual([
      "capacityBasis",
      "capacityPerUnit",
      // Контекст задачи — такое же входное поле движка, как поток нагрузки: без него замещение
      ...
      "maintenanceUsdYear",
      "priceUsd",
      // Поток нагрузки — теперь такое же входное поле движка, как базис ёмкости: без него
      // расчёт сравнивает несравнимое.
      "workerOutputPerYear",
      "workloadStream",
    ]);
  });
