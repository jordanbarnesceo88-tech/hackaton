...
написано в комментарии над ними. После задачи 6 движок их не читает, и перебор даёт размах
ровно 0 ₽ (подтверждено прототипом, оба потока).

**Важное уточнение, полученное прогоном, а не чтением: рычаг мёртв УЖЕ СЕЙЧАС.** Торнадо на
нынешнем движке, пищевое производство:

```
штат   80: предел без потолка 300, замещено  80, размах          0  ← потолок штата съедает перебор
штат  400: предел без потолка 300, замещено 300, размах  7 786 317  ← рычаг жив
```

`displacedFte = min(staffCount, спрос/выработка)`: когда связывает потолок штата — а по
стресс-прогону это частый случай для потока операций — перебор делителя не двигает ничего.
Человек видит столбец нулевой длины уже сегодня.

Значит задача не «не внести регрессию», а **убрать существующую**. Смена формулы делает
поломку структурной вместо сценарно-зависимой, и это упрощает: структурно мёртвый рычаг
удаляется, сценарно мёртвый требует решения на отрисовке — см. шаг 1.

**Второй мёртвый рычаг, к подпроекту A отношения не имеющий:** `assetLifeYears` даёт размах 0
в обоих потоках уже сейчас. При горизонте 5 лет и сроке службы 7 перебор ±1 год (6 и 8) не
пересекает границу повторного CAPEX внутри горизонта. Существует независимо от нас, но
попадает в ту же задачу, потому что лечится тем же решением.

- [ ] **Шаг 1: Написать тесты**

Абсолютный инвариант «ни один рычаг не имеет нулевого размаха» **написать нельзя** — он упал бы
сегодня на `assetLifeYears`, до единой нашей правки. Проверено прогоном. Вместо него два теста:

```ts
it("делители предела замещения больше не считаются рычагами", () => {
  // После подпроекта A движок их не читает — структурно, при любом сценарии. Рычаг,
  // который не может двигать NPV, в диаграмме ранжирования не место.
  for (const stream of ["OPERATION_FLOW", "FLOOR_AREA"] as const) {
    const keys = sensitivity({ ...cap, workloadStream: stream }, params, DEFAULT_ASSUMPTIONS, 6).map((b) => b.key);
...
it("занятость по задаче присутствует среди рычагов", () => {
  // Со-доминирующий рычаг: занятость и ставка труда входят в годовую экономию линейно и
  // мультипликативно, поэтому их размахи совпадают до рубля (проверено: оба 145 993 на
  // сценарии пищевого производства). Диаграмма без занятости пропускает половину ответа
  // на вопрос «от чего это число зависит сильнее всего».
  const bars = sensitivity(cap, params, DEFAULT_ASSUMPTIONS, 6);
  expect(bars.map((b) => b.key)).toContain("taskFte");
});

it("рычаг с нулевым размахом помечен, а не выброшен и не выглядит обычным", () => {
  // assetLifeYears при горизонте 5 и сроке службы 7 даёт размах 0: перебор ±1 год не
  // пересекает границу повторного CAPEX. Выбрасывать нельзя — в sensitivity.ts стоит
  // комментарий, что молча исчезающий столбец уже был багом. Поэтому: показывать
  // погашенным с подписью «не влияет при этих параметрах».
  const bar = sensitivity(cap, params, DEFAULT_ASSUMPTIONS, 6).find((b) => b.key === "assetLifeYears");
  expect(bar, "столбец не должен исчезать молча").toBeDefined();
  expect(bar!.swing).toBe(0);
  expect(bar!.inert, "нулевой размах обязан быть помечен для отрисовки").toBe(true);
});
```

- [ ] **Шаг 2: Запустить, убедиться что падает**

Ожидается три падения: `opsPerWorkerPerYear` присутствует, `taskFte` отсутствует, поля `inert`
нет.
...
- [ ] **Шаг 4: Добавить занятость рычагом и пометку инертности**

Расширить `SensitivityBar.key` до `keyof AssumptionValues | "taskFte"`, перебирать `taskFte`
±25% наравне с процентными допущениями, подпись «Занято на задаче, человек». Добавить в
`SensitivityBar` поле `inert: boolean` (`swing === 0`), чтобы отрисовка могла погасить столбец,
