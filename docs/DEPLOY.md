...
**До миграции три последних пункта приблизительны, и preflight об этом говорит.** Личность
решения — `Solution.slug`, а этой колонки на боевой ещё нет: её заводит
`20260913120000_solution_slug`. Пока её нет, переезд между категориями и настоящий дубликат
отсюда неразличимы (признак один — имя), поэтому пункт про дубликаты здесь — предупреждение,
а не `[СТОП]`: блокировать выкат за операцию, которую `slug` как раз и разрешает, было бы
неверно. Точный ответ даёт шаг 3.5a.
...
DATABASE_URL="$PGDIRECT" npm run preflight:deploy
```
Теперь `[СТОП]` не должно остаться ни одного, а `[ВНИМ]` останутся два ожидаемых:
«Пользовательские данные в базе» и — если бэкфилл миграции опознал не всё — «строки со
`slug`'ом `legacy-…`», которые чистка удалить не смогла.

Оставшийся `[СТОП]` означает одно из двух, и лечение у них РАЗНОЕ:
