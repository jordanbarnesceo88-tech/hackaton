...
|---|---|---|
| паллетирование | коробок/час | **НЕ НАЙДЕНО** |
| уборка | м²/час | есть, помечено «порядок величины, а не цитата» (600 000 м²/год) |
| отбор заказов | отборов/час | не искали — кончились кредиты |
| инспекция качества | деталей/час | не искали — кончились кредиты |
| сортировка | посылок/час | не искали — кончились кредиты |
| доставка | доставок/смена | не искали — кончились кредиты |

**Паллетирование: искали, не нашли.** Запрос про норму выработки ручной укладки возвращает
геометрию паллеты — сколько коробок помещается на европоддон, высота штабеля, распределение
веса — но не производительность человека в час. Открытых нормативов по этой операции в
доступном виде нет.
...
не будет посчитана». На сегодня пустых больше, чем заполненных.
