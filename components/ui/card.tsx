...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // shadow-panel: BCB's panel elevation (docs/design/PROTOTYPE-DESIGN-SYSTEM.md), additive
        // to the existing ring rather than replacing it — the ring is what carries the edge in
        // forced-colors/high-contrast modes, where box-shadow is dropped.
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-xl bg-card py-(--card-spacing) text-sm text-card-foreground ring-1 ring-foreground/10 shadow-panel [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
        className
      )}
      {...props}
    />
